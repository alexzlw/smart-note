...
const App: React.FC = () => {
  ...
  // Load data on mount from IndexedDB
  const loadData = async () => {
    try {
      setIsLoading(true);
      const data = await dbService.getAllMistakes();
      setMistakes(data);
    } catch (error) {
  ...
  const handleAddMistake = async (item: MistakeItem) => {
    try {
        ...
        await dbService.addMistake(item);
        setView('review'); // Switch to review view to see the new item
    } catch (e) {
  ...
  const handleUpdateItem = async (updatedItem: MistakeItem) => {
     try {
        ...
        await dbService.updateMistake(updatedItem);
     } catch (e) {
  ...
  const handleUpdateMastery = async (id: string, level: MasteryLevel) => {
    try {
        ...
        await dbService.updateMistake(newItem);
    } catch (e) {
  ...
  const handleDeleteMistake = async (id: string) => {
      ...
      if (window.confirm("この間違いノートを削除しますか？\n（この操作は元に戻せません）")) {
          try {
              await dbService.deleteMistake(id);
      ...
      return (
          <div className="min-h-screen flex items-center justify-center bg-slate-50">
              <div className="flex flex-col items-center gap-4">
                  ...
                  <p className="text-slate-500 font-medium">ノートブックを読み込み中...</p>
              </div>
          </div>
      );
  }
                ...
                <div className="space-y-8 animate-fade-in">
                    ...
                    {filteredMistakes.length === 0 ? (
                        <div className="flex flex-col items-center justify-center py-24 bg-white rounded-3xl border border-dashed border-slate-200 text-center shadow-sm">
                            ...
                            <p className="text-slate-500 max-w-sm mx-auto mb-8">現在のフィルターに一致する問題はありません。</p>
                            <button 
