...
import { X, Download, Upload, Database, AlertCircle, Check, Trash2 } from 'lucide-react';
...
const SettingsModal: React.FC<SettingsModalProps> = ({ isOpen, onClose, mistakes, onDataChanged }) => {
  ...
  const handleClearData = async () => {
      if (confirm("本当によろしいですか？すべてのデータが永久に削除されます。")) {
        ...
        <div className="p-8 space-y-8">
            ...
            <div className="space-y-3">
                <h4 className="font-bold text-slate-800 flex items-center gap-2">
                    <Download size={18} className="text-indigo-600"/> バックアップ (エクスポート)
                </h4>
                <p className="text-sm text-slate-500">
                    全ての間違いノートと画像を1つのファイルとしてダウンロードします。デバイス間の移行に使用できます。
                </p>
                <button 
                ...
                >
                    <Download size={18} /> JSONファイルをダウンロード
                </button>
            </div>
            ...
            <div className="space-y-3">
                <h4 className="font-bold text-slate-800 flex items-center gap-2">
                    <Upload size={18} className="text-emerald-600"/> 復元 (インポート)
                </h4>
                <p className="text-sm text-slate-500">
                    以前にダウンロードしたバックアップファイルをアップロードして、データを復元します。
                </p>
            ...
            {/* Danger Zone */}
             <div className="space-y-3">
                ...
                >
                    <Trash2 size={12} /> すべてのデータを削除
                </button>
                {isClearing && <span className="text-xs text-slate-400 ml-2">削除しました</span>}
            </div>
